...
import { fetchTeams, fetchTeamMembers, removeTeamMemberThunk, deleteTeamThunk, updateTeamMemberPermissionsThunk } from "@/store/teamSlice";
import { fetchUsers, fetchCurrentUser } from "@/store/userSlice";
import { fetchPermissions } from "@/store/permissionSlice";
import { getMemberById, computeNextPermissionIds } from "./helpers";

const TeamsPage = () => {
  const dispatch = useDispatch();
  const router = useRouter();
  const {membersByTeam, loading, error, items: teams} = useSelector((s) => s.teams);
  const { items: users } = useSelector((s) => s.users);
  const { current: user, currentToken } = useSelector(
    (s) => s.users || {}
  );
  ...
  const ensureMembersLoaded = useCallback((teamId) => {
  ...
  const handleRemoveMember = useCallback((teamId, userId) => {
    if (!teamId || !userId) return Promise.resolve();
    const key = `${teamId}:${userId}`;
    setRemovingMember(key);
    return dispatch(removeTeamMemberThunk({ teamId, userId }))
  ...
  return (
    <div className="p-6">
      ...
      {/* Toolbar */}
      <TeamsToolbar onCreate={() => setOpen(true)} />
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  <button
                    onClick={() => { ensureAllMembersLoaded(); ensureMembersLoaded(t.id); setMemberModalFor(t); }}
                ...
                <div className="px-4 pb-4">
                  ...
                  <TeamMembersList
                    ...
                    onRemoveMember={(teamId, userId) => {
                      const member = getMemberById(membersByTeam, teamId, userId);
                    ...
                    onTogglePermission={(teamId, userId, permKey, enable) => {
                      ...
                      const member = getMemberById(membersByTeam, teamId, userId);
                      const nextIds = computeNextPermissionIds(permList, member, permKey, enable);
                      if (!nextIds) return;
                      dispatch(updateTeamMemberPermissionsThunk({ teamId, userId, permissionIds: nextIds }));
                    }}
                  />
                </div>
              )}
      ...
      <ConfirmDialog
        ...
        busy={
          !!(
            confirmRemove &&
            removingMember === `${confirmRemove.team?.id}:${confirmRemove.member?.id}`
          )
        }
